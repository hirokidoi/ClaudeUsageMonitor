...
「失敗」には JS 実行エラー・更新ボタン未検出・API レスポンスのタイムアウト（10 秒以内に `/usage` API の応答が届かない場合）・プラン判定不能・JSON パース失敗・ファイル書き込み失敗・API POST 失敗（HTTP 非 200 系・通信エラー）などを含む。API エンドポイント URL 未設定やファイル名未設定は skip 扱いで失敗にはカウントしない。
